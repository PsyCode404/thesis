...
The matching problem is reformulated from a simple comparison task into a geometric optimization problem.

Instead of checking whether two rides share identical origins and destinations, the system evaluates whether a passenger can be integrated into a driver’s trajectory under feasible conditions. This reflects real-world ride-sharing scenarios, where routes often overlap partially rather than exactly.

This reformulation introduces two main challenges:

\begin{itemize}
    \item efficient detection of spatial proximity between trajectories,
    \item identification of pickup points that minimize route disruption.
\end{itemize}

A major challenge lies in balancing flexibility and feasibility. Expanding the search space increases potential matches but may also produce impractical route modifications.

The proposed approach therefore avoids relying on simple point-to-point distance comparisons, which ignore road-network structure and realistic travel constraints.

Instead, candidate rides are evaluated through a multi-stage scoring process~\cite{routing}. Passenger locations are projected onto the driver’s route, detour cost is estimated, and candidates are ranked using spatial and temporal criteria.

As a result, the problem becomes a constrained route-insertion task whose objective is to identify feasible pickup opportunities while preserving route continuity and limiting additional travel cost.

Alternative methods such as nearest-point matching or grid-based partitioning were considered. However, these approaches either oversimplify route structure or introduce excessive approximation. The corridor-based strategy provides a more balanced solution in terms of flexibility, accuracy, and computational efficiency.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.99\textwidth]{images/architecture/route_matching.png}
...
The corridor width is dynamically adjusted according to expected travel conditions and tolerated deviation constraints. Instead of using a fixed spatial radius, the system adapts the search corridor based on estimated vehicle speed and maximum acceptable detour duration. This allows the filtering process to remain both flexible and computationally efficient across different ride scenarios.
...
where $v$ represents the estimated average vehicle speed, $\delta$ the maximum tolerated detour time in minutes, and $w_{\min}$ the minimum corridor width maintained to avoid excessively restrictive filtering.
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{images/architecture/matching_pipeline.png}
...
The pickup proximity factor is itself computed using a composite formulation combining passenger accessibility and estimated detour impact. This allows the system to balance route convenience for the driver with spatial accessibility for the passenger.

\begin{equation}
d_p =
\frac{1}{2}
\left[
\max\left(0,1-\frac{D_r}{5}\right)
+
\max\left(0,1-\frac{T_{det}}{10}\right)
\right]
\end{equation}

where $D_r$ denotes the passenger-to-route distance in kilometers and $T_{det}$ represents the estimated detour time in minutes. The formulation penalizes both excessive spatial deviation and excessive route disruption.
...
Unlike traditional ride-sharing systems where pickup points are predefined, the proposed approach determines pickup locations dynamically.

The selected point aims to minimize detour cost while remaining accessible to both driver and passenger. This optimization considers route continuity, travel feasibility, and geographic constraints.

A major design objective is preserving driver convenience. Since drivers are not financially incentivized, limiting additional route deviation is essential for maintaining participation and ensuring practical ride execution.

To support decision-making, the system computes and presents the estimated detour associated with each proposed match, allowing users to evaluate the impact before confirming the ride.

% ------------------------
\subsection{Ride Lifecycle Adaptation}

The matching mechanism is tightly integrated into the ride lifecycle.

Rather than treating matching as a static preprocessing step, the system dynamically adapts ride execution as new participants are accepted. A ride initially planned as A → C may evolve into a multi-stage process:
...
This reflects the dynamic nature of real-world ride-sharing, where routes may evolve during execution without requiring complete trip redefinition.

From a system perspective, the lifecycle model must support intermediate states and synchronized transitions. Each accepted match updates ride state, participant information, and route progression while preserving execution consistency across the platform.

As a result, the lifecycle becomes an active coordination component linking route planning, matching, and real-time ride execution.
...
\begin{minipage}[c]{0.36\textwidth}
    \centering
    \includegraphics[width=0.85\textwidth]{images/ui/smart_notification_preview.png}
    \caption*{(a) Smart Notification Preview}
\end{minipage}
\hspace{0.03\textwidth}
\begin{minipage}[c]{0.36\textwidth}
    \centering
    \includegraphics[width=0.85\textwidth]{images/ui/smart_match_screen.png}
...
To ensure realistic spatial clustering, geographic proximity between sampled points is measured using the Haversine formula, which computes the great-circle distance on a spherical surface:
...
where $R$ denotes the Earth radius, $\varphi$ represents latitude, and $\lambda$ represents longitude. This approach accounts for the curvature of the Earth and ensures that spatial clustering reflects realistic geographic distances rather than simplified Euclidean approximations.
...
Before entering the prediction stage, each candidate pattern is assigned a confidence score that balances repetition frequency and temporal consistency:
...
\text{confidence} =
0.6 \cdot
\min\left(1,\frac{n}{20}\right)
+
0.4 \cdot
\max\left(0,1-\frac{\sigma_t}{60}\right)
\end{equation}

where $n$ denotes the number of observed trips and $\sigma_t$ the standard deviation of departure times in minutes. This formulation ensures that patterns with insufficient observations or excessive temporal variability are filtered out, preserving only stable and well-supported behavioral signals.
...
To quantify temporal regularity, a consistency score is computed from the distribution of departure times associated with each pattern:

\begin{equation}
\text{time\_consistency} =
\frac{1}{1+\sigma_t}
\end{equation}

where $\sigma_t$ denotes the standard deviation of departure times in minutes, consistent with the definition used in the confidence score formulation.
...
The Logistic Regression model estimates commute probability through a linear combination of features passed through a sigmoid function:

\begin{equation}
P(y=1 \mid \mathbf{x}) =
\frac{1}{
1 + e^{-(\mathbf{w}^{\top}\mathbf{x}+b)}
}
\end{equation}

where $\mathbf{x}$ represents the feature vector, $\mathbf{w}$ the learned weights, and $b$ the bias term. Because this model assumes a linear relationship between inputs and the log-odds of the output, it cannot capture complex non-linear interactions between temporal, spatial, and frequency features. This structural limitation explains its lower predictive performance compared to Random Forest, which partitions the feature space recursively to model richer decision boundaries.
...
The F1-score is used to evaluate the balance between precision and recall, and is defined directly in terms of true positives (TP), false positives (FP), and false negatives (FN):
...
This chapter presented an intelligent decision layer that extends the platform beyond purely reactive behavior, enabling a more adaptive and proactive system.

The Smart Matching System improves conventional ride-sharing mechanisms by treating rides as continuous trajectories rather than fixed origin--destination pairs. Through dynamic pickup insertion and route-aware matching, the system increases matching opportunities while preserving route feasibility.

Alongside this component, the Machine Learning System introduces predictive capabilities based on recurring commuting patterns. By generating contextual ride suggestions, the platform reduces repetitive user interaction and improves overall usability while remaining independent from the core ride management logic.

Combined together, these mechanisms create a hybrid architecture where deterministic matching ensures operational consistency and predictive models enhance adaptability and personalization.

Overall, the platform progresses from a basic transactional application toward a synchronized and intelligent transportation system capable of reacting to and anticipating user needs.

The following chapter presents the global evaluation and validation of the proposed system, including performance analysis, matching effectiveness, prediction relevance, and identified limitations.